...
for i, nl in enumerate(noise_levels):
    for j, nc in enumerate(num_cameras):
        ...
        #break 
    #break 
fig.suptitle(f"{num_cameras}, {noise_levels}")
plt.tight_layout()
...
save_name = "analysis_results/skull_values.json"
...
noises = np.asarray([1, 5, 10, 15, 18, 20])
colors = ["black", "blue", "green", "gray", "orange", "red"]
used_colors = []
for key, item in experiment_dict.items():
    if key == base_key:
        continue
    ...
    plt.scatter(num, score, color=color, label=label)
...
for noise, color in zip(noises, colors): 
    ...
    for key, item in experiment_dict.items():
        ...
        value = value / count 
        plt.scatter([num_cameras], [value], color=color)
plt.xlabel("# Cameras") 
plt.ylabel("Mean Error (mm)")
